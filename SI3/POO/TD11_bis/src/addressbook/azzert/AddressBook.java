 ...
 * indexed by both name and phone number. This version uses assert statements to
...
class AddressBook {
    ...
    void addDetails(ContactDetails details) {
        if (details == null) {
            throw new IllegalArgumentException(
                    "Null details passed to addDetails.");
        }
     ...
     * @param details
     *            The replacement details. Must not be null.
    ...
    /**
     * Check that the numberOfEntries field is consistent with the number of
     * entries actually stored in the address book.
     * 
    ...
    private boolean consistentSize() {
        ...
        Set<ContactDetails> uniqueEntries = new HashSet<>(
                allEntries);
